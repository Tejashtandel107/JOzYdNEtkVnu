...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.dashboard.index', $data)->with(compact('request','customers','data')); 

    }
    public function show(Request $request){
        $request->merge(["r" => [0 => "new",1 => "final"]]);
        ...
        return view('admin.dashboard.includes.index')->with(compact('request','stock_statistics','additional_charge','outward_total_amount','insurance_statistics','calculate_stocks')); 
    }
    public function showOutstandingPayment(Request $request){

        $outstanding_payments = $this->orderitem_obj->getOutstandingPayment($request);

        return view('admin.dashboard.includes.outstanding-payment')->with(compact('request','outstanding_payments')); 
    }
    /**
