...
<div class="page-content fade-in-up">
    <div class="ibox mb-2">
        <div class="ibox-body py-3">
            <?php echo e(Form::model($request, array('url' =>route('admin.trash.orders'),'method'=>'get','id'=>'form-filter','autocomplete'=>'off'))); ?>
                ...
                <?php if(isset($customer_orders) && $customer_orders->count()>0): ?>
                    <?php $__currentLoopData = $customer_orders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $customer_order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <tr>
                            ...
                            <td><?php echo e($customer_order->type); ?></td>
                            <td><?php echo e(($customer->fullname) ?? "N/A"); ?></td>                            
                            <td><?php echo e($customer_order->additional_charge); ?></td>
                            <td><?php echo e($customer_order->firstname); ?> <?php echo e($customer_order->lastname); ?></td>
                            <td>
                                <button class="btn btn-outline-success btn-icon-only btn-sm mb-2" onclick="restoreTrashOrder( '<?php echo e(route('admin.trash.orders.restore',$customer_order->customer_order_id)); ?>','<?php echo e($customer_order->customer_order_id); ?>' );" title="Restore"><i class="la la-recycle"></i></button> &nbsp;
                            <?php if($customer_order->type == 'inward'): ?>
                            ...
                            <?php elseif($customer_order->type == 'outward'): ?>
                                <a href="<?php echo e(route('admin.outwards.edit',$customer_order->customer_order_id)); ?>" title="Edit"><button class="btn btn-outline-info btn-icon-only btn-sm mb-2"><i class="la la-pencil"></i></button></a> &nbsp;
                                <a href="<?php echo e(route('admin.outwards.showReceipt',$customer_order->customer_order_id)); ?>" title="Receipt"><button class="btn btn-outline-dark btn-icon-only btn-sm mb-2"><i class="ti ti-printer"></i></button></a> &nbsp;                                  
                            <?php endif; ?>                                
                                <button class="btn btn-outline-danger btn-icon-only btn-sm mb-2" onclick="deleteTrashOrder( '<?php echo e(route('admin.trash.orders.destroy',$customer_order->customer_order_id)); ?>','<?php echo e($customer_order->customer_order_id); ?>' );" title="Delete"><i class="la la-trash"></i></button> &nbsp;
                            </td>
                        </tr>
...
<?php $__env->startSection('page-scripts'); ?>
    <?php echo Html::script('/assets/admin/js/trash/index.js'); ?>

<?php $__env->stopSection(); ?>

<?php echo $__env->make('admin.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/gurukrupafoodproducts/application/resources/views/admin/trash/order.blade.php ENDPATH**/ ?>